...
The adapter downloads the published Parquet artifact directly and loads the
local file. This avoids both the legacy ``massive.py`` dataset script and
...
def download_parquet(destination: Path) -> None:
    """Download the pinned MASSIVE Parquet artifact to a local temporary path."""
