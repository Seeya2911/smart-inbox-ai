...
class SmartSummarizerV3:
    ...
    PLATFORM_LIMITS = {"whatsapp": 50, "email": 100, "slack": 60, "teams": 80, "instagram": 40, "discord": 60}

    def __init__(self, context_file: str = "message_context.json", max_context_messages: int = 3, confidence_threshold: float = 0.6) -> None:
        self.context_file = context_file
        self.max_context_messages = max_context_messages
        self.confidence_threshold = confidence_threshold
        self.context_data = self._load_context()
        self.stats: Dict[str, Any] = {"processed": 0, "context_used": 0, "platforms": {}, "intents": {}, "urgency_levels": {}, "unique_users": set()}
    ...
    def _cleanup_old_context(self, data: Dict[str, Any]) -> None:
        ...
        for key, messages in data.get("conversations", {}).items():
            data["conversations"][key] = [m for m in messages if self._parse_datetime(m.get("timestamp")) > cutoff]
    ...
    def _extract_context(self, user_id: str, platform: str) -> List[Dict[str, Any]]:
        messages = self.context_data.get("conversations", {}).get(self._get_context_key(user_id, platform), [])
    ...
    def _store_message_context(self, message_data: Dict[str, Any]) -> None:
        key = self._get_context_key(str(message_data.get("user_id", "unknown")), str(message_data.get("platform", "unknown")))
        conversations = self.context_data.setdefault("conversations", {})
        history = conversations.setdefault(key, [])
        history.append({"message_text": str(message_data.get("message_text", "")), "timestamp": message_data.get("timestamp") or datetime.now(timezone.utc).isoformat(), "message_id": message_data.get("message_id") or f"msg_{datetime.now().timestamp()}"})
    ...
    def _classify_intent(self, text: str, context_messages: Optional[List[Dict[str, Any]]] = None) -> Tuple[str, float]:
        ...
        request_patterns = [
            ...
            r"\bplease\s+\w+", r"\bsend me\b", r"\bi need\b", r"\bneed (?:you|a|the|this|that)\b",
        ]
        if any(re.search(pattern, normalized) for pattern in request_patterns):
            return "request", 0.9

        if context_messages and any(token in normalized for token in ("update", "status", "progress", "any news", "follow up", "follow-up")):
            current_words = set(re.findall(r"\b\w+\b", normalized))
            for previous in context_messages:
                previous_words = set(re.findall(r"\b\w+\b", str(previous.get("message_text", "")).lower()))
    ...
    def _analyze_urgency(self, text: str, context_messages: Optional[List[Dict[str, Any]]] = None) -> Tuple[str, float]:
        ...
        if any(token in normalized for token in high): return "high", 0.95
        if any(token in normalized for token in medium): return "medium", 0.75
        if any(token in normalized for token in low): return "low", 0.9
        if context_messages:
            ...
            if current_pressure > previous_pressure: return "high", 0.8
        if len(text) < 50: return "low", 0.4
        if "?" in text or any(word in normalized for word in ("need", "want", "require")): return "medium", 0.5
        return "low", 0.3
    ...
    def summarize(self, message: Dict[str, Any], use_context: bool = True) -> Dict[str, Any]:
        ...
        result = {"summary": self._make_summary(text, self.PLATFORM_LIMITS.get(platform, 100)), "intent": intent, "urgency": urgency, "confidence": float(min(intent_confidence, urgency_confidence)), "intent_confidence": intent_confidence, "urgency_confidence": urgency_confidence, "context_used": context_used, "platform_optimized": True, "platform": platform, "context_insights": ["Related conversation context was available."] if context else [], "message_id": message.get("message_id") or f"msg_{datetime.now().timestamp()}", "baseline": True}
        ...
        if use_context: self._store_message_context(message)
        return result
